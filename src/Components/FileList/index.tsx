...
export default function FileList({ files }: IFileListProps) {
    ...
    return (
        <ul className="flex flex-col w-11/12 md:w-9/12 m-auto  justify-between items-center  cursor-pointer">
            {files?.map(file => (
                <li key={file.id} className="flex bg-surface-100  hover:bg-surface-100/50 mb-2  w-full  justify-between items-center p-6 border border-surface-300 rounded-2xl shadow-sm">
