...
export default function ButtonHamburguer({ onClick }: ButtonHamburguerProps) {
    ...
    return (
        <button onClick={toggleValue} className={`md:hidden space-y-2 ${toogleMenuValue ? 'absolute z-50' : ''} relative group`}>
            <div className="relative flex overflow-hidden items-center justify-center rounded-full w-[50px] h-[50px] transform transition-all bg-slate-700 ring-0 ring-gray-300 duration-200 text-white">
