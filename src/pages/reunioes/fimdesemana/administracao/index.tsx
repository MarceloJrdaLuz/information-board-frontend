...
import { addAndUpdateSpeakerCoordinateAtom, addAndUpdateWatchtowerConductorAtom } from "@/atoms/systemCongregationAtoms"
...
import { BookOpen, CheckCircle2, Phone, Save, ShieldCheck, User, UserCheck } from "lucide-react"
import { useEffect, useState } from "react"
import { toast } from "react-toastify"

function AdminWeekendArrangement() {
    ...
    const [currentCoordinator, setCurrentCoordinator] = useState<IPublisher | null>(user?.congregation?.speakerCoordinator || null)
    const [isSavingSpeaker, setIsSavingSpeaker] = useState(false)

    // Dirigente de A Sentinela
    const sendWatchtowerConductor = useSetAtom(addAndUpdateWatchtowerConductorAtom)
    const [watchtowerConductor, setWatchtowerConductor] = useState<IPublisher | null>(user?.congregation?.watchtowerConductor || null)
    const [currentWatchtowerConductor, setCurrentWatchtowerConductor] = useState<IPublisher | null>(user?.congregation?.watchtowerConductor || null)
    const [isSavingWatchtower, setIsSavingWatchtower] = useState(false)
    ...
    useEffect(() => {
        setPageActive('/reunioes/fimdesemana/administracao')
        setCrumbs([
    ...
    useEffect(() => {
        if (user?.congregation) {
            if (user.congregation.speakerCoordinator !== undefined) {
                setSpeakerCoordinator(user.congregation.speakerCoordinator)
                setCurrentCoordinator(user.congregation.speakerCoordinator)
            }
            if (user.congregation.watchtowerConductor !== undefined) {
                setWatchtowerConductor(user.congregation.watchtowerConductor)
                setCurrentWatchtowerConductor(user.congregation.watchtowerConductor)
            }
        }
    }, [user?.congregation])

    function onSubmitSpeaker() {
        ...
        if (user?.congregation) {
            ...
            toast.promise(sendSpeakerCoordinator(user.congregation.id, speakerCoordinator.id), {
                ...
                setCurrentCoordinator(speakerCoordinator)
                mutate()
            }).catch(err => {
                console.error(err)
            }).finally(() => {
    ...
    function onSubmitWatchtower() {
        if (!watchtowerConductor) {
            toast.warning("Por favor, selecione um publicador.")
            return
        }

        if (user?.congregation) {
            setIsSavingWatchtower(true)
            toast.promise(sendWatchtowerConductor(user.congregation.id, watchtowerConductor.id), {
                pending: 'Definindo dirigente da sentinela...',
                success: 'Dirigente da sentinela atualizado com sucesso!',
                error: 'Erro ao atualizar dirigente da sentinela.'
            }).then(() => {
                setCurrentWatchtowerConductor(watchtowerConductor)
                mutate()
            }).catch(err => {
                console.error(err)
            }).finally(() => {
                setIsSavingWatchtower(false)
            })
        }
    }
    ...
    return (
        <ContentDashboard>
            ...
            <section className="flex flex-col w-full min-h-full p-3 sm:p-5 md:p-6 gap-8 max-w-7xl mx-auto">
                {/* ==================================================== */}
                {/* HERO CARD                                            */}
                {/* ==================================================== */}
                <div className="flex flex-col gap-4 bg-surface-100 border border-surface-300 rounded-2xl p-4 sm:p-5 shadow-sm">
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-primary-200 font-semibold text-xs uppercase tracking-wider">
                            ...
                            <span>Arranjos • Reunião de Fim de Semana</span>
                        </div>
                        <h1 className="text-xl sm:text-2xl font-bold text-typography-900">
                            Administração da Reunião de Fim de Semana
                        </h1>
                        <p className="text-xs sm:text-sm text-typography-500">
                            Defina e gerencie os irmãos designados para coordenar o arranjo de oradores e dirigir o Estudo de A Sentinela da congregação.
                        </p>
                    </div>
                </div>

                {/* ==================================================== */}
                {/* SEÇÃO 1: COORDENADOR DE DISCURSOS PÚBLICOS           */}
                {/* ==================================================== */}
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2 px-1">
                        <ShieldCheck className="h-5 w-5 text-primary-200" />
                        <div>
                            <h2 className="text-lg font-bold text-typography-900">Arranjo de Oradores Públicos</h2>
                            <p className="text-xs text-typography-500">Coordenação de discursos públicos e hospitalidade</p>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-4 bg-surface-100 border border-surface-300 rounded-2xl p-5 shadow-sm">
                            <div className="flex items-center justify-between pb-3 border-b border-surface-300">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-base font-bold text-typography-900">Coordenador Atual</h3>
                                </div>
                                {currentCoordinator ? (
                        ...
                        <div className="flex flex-col gap-4 bg-surface-100 border border-surface-300 rounded-2xl p-5 shadow-sm">
                            ...
                            <div className="pt-3 border-t border-surface-300 flex justify-end">
                                ...
                                >
                                    ...
                                    <span>{isSavingSpeaker ? "Salvando..." : "Salvar Coordenador"}</span>
                                </Button>
                            </div>
                        </div>
                    </div>
                ...
                {/* ==================================================== */}
                {/* SEÇÃO 2: DIRIGENTE DE A SENTINELA                    */}
                {/* ==================================================== */}
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2 px-1">
                        <BookOpen className="h-5 w-5 text-primary-200" />
                        <div>
                            <h2 className="text-lg font-bold text-typography-900">Estudo de A Sentinela</h2>
                            <p className="text-xs text-typography-500">Dirigente responsável pelo Estudo de A Sentinela</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                        {/* Card: Dirigente Atual */}
                        <div className="flex flex-col gap-4 bg-surface-100 border border-surface-300 rounded-2xl p-5 shadow-sm">
                            <div className="flex items-center justify-between pb-3 border-b border-surface-300">
                                <div className="flex items-center gap-2">
                                    <UserCheck className="h-5 w-5 text-primary-200" />
                                    <h3 className="text-base font-bold text-typography-900">Dirigente Atual</h3>
                                </div>
                                {currentWatchtowerConductor ? (
                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800">
                                        <CheckCircle2 className="h-3.5 w-3.5" />
                                        Ativo
                                    </span>
                            ...
                            {currentWatchtowerConductor ? (
                                <div className="flex items-center gap-4 p-4 rounded-xl bg-surface-200/50 border border-surface-300/80">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-base font-bold text-typography-900">
                                            {currentWatchtowerConductor.fullName}
                                        </span>
                                        {currentWatchtowerConductor.phone && (
                                            <div className="flex items-center gap-1.5 text-xs text-typography-600 mt-1">
                                                <Phone className="h-3.5 w-3.5" />
                                                <span>{currentWatchtowerConductor.phone}</span>
                                            </div>
                                        )}
                                        <span className="text-[11px] text-typography-500 mt-1">
                                            Responsável por dirigir o Estudo de A Sentinela semanal da congregação.
                                        </span>
                                    </div>
                                </div>
                            ) : (
                                <div className="p-6 text-center text-sm text-typography-500 bg-surface-200/40 rounded-xl border border-dashed border-surface-300">
                                    Nenhum dirigente de A Sentinela designado no momento. Selecione um irmão ao lado para designar.
                                </div>
                            )}
                        </div>

                        {/* Card: Designar / Alterar Dirigente */}
                        <div className="flex flex-col gap-4 bg-surface-100 border border-surface-300 rounded-2xl p-5 shadow-sm">
                            <div className="flex items-center gap-2 pb-3 border-b border-surface-300">
                                <BookOpen className="h-5 w-5 text-primary-200" />
                                <h3 className="text-base font-bold text-typography-900">
                                    {currentWatchtowerConductor ? "Alterar Dirigente" : "Designar Dirigente"}
                                </h3>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 mt-1">
                                ...
                                <DropdownObject
                                    ...
                                    title="Selecione um irmão"
                                    items={filteredPublishersMan ?? []}
                                    selectedItem={filteredPublishersMan?.find(p => p.id === watchtowerConductor?.id) ?? null}
                                    handleChange={item => setWatchtowerConductor(item)}
                                    labelKey="fullName"
                                    border
                                    full
                                    emptyMessage="Nenhum publicador qualificado encontrado"
                            ...
                            <div className="pt-3 border-t border-surface-300 flex justify-end">
                                <Button
                                    onClick={onSubmitWatchtower}
                                    disabled={isSavingWatchtower}
                                    className="text-typography-200 px-5 py-2.5 rounded-xl flex items-center gap-2 font-semibold shadow-sm"
                                >
                                    <Save className="h-4 w-4" />
                                    <span>{isSavingWatchtower ? "Salvando..." : "Salvar Dirigente"}</span>
                                </Button>
                            </div>
                        </div>
                    </div>
...
AdminWeekendArrangement.getLayout = withProtectedLayout(["ADMIN_CONGREGATION"])

export default AdminWeekendArrangement
