...
function Limpeza() {
    ...
    const urlSchedulesConfig = congregation
        ...
        : "";
    const { data, mutate } = useFetch<ICleaningScheduleResponse>(urlSchedulesConfig);
        ...
        <div className=" flex flex-col h-screen w-screen bg-typography-200">
            ...
            <div className=" flex flex-col h-screen w-screen bg-typography-200 overflow-auto">
                <LayoutPrincipal
                    ...
                    <div className="flex flex-col overflow-auto hide-scrollbar w-11/12 md:w-8/12 gap-2 my-2 m-auto">
                        {/* 1) Se existir programação no banco → mostra o carousel */}
                        {data?.schedules && data.schedules.length > 0 ? (
                            <CleaningScheduleCarousel schedule={data} />
                        ) : (
                            <>
                                {/* 2) Se não tiver schedule → tenta mostrar PDFs */}
                                {documents ? (
